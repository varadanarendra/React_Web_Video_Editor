...
import { addSegment, deleteSegment, selectAllSegments } from './features/segments/segmentsSlice';
import { addOverlay, deleteOverlay, selectAllOverlays } from './features/overlays/overlaysSlice';
import { selectPlayhead, setPlaying, setPlayheadTime, fitTimelineToContent, selectSelection, clearSelection } from './features/ui/uiSlice';
...
function App() {
  ...
  useEffect(() => {
    const loadVideosFromPublic = async () => {
      ...
      for (const { file: videoFile, label: videoLabel } of videoFiles) {
        ...
        await new Promise((resolve) => {
          const timeout = setTimeout(() => {
            console.warn(`Timeout loading video: ${videoFile} from ${videoPath}`);
          ...
          video.onloadedmetadata = () => {
            ...
            if (duration && duration > 0 && !isNaN(duration)) {
              console.log(`Loaded video: ${videoLabel} (${videoFile}) - Duration: ${duration}s`);
              dispatch(addSegment({
                source: videoPath,
                label: videoLabel,
                duration: duration,
              ...
              console.warn(`Invalid duration for video: ${videoFile}`, duration);
            }
            resolve();
          };
          ...
          video.onerror = (e) => {
            clearTimeout(timeout);
            console.error(`Failed to load video: ${videoFile} from ${videoPath}`, e);
      ...
      console.log(`Finished loading videos. Successfully loaded ${loadedCount} out of ${videoFiles.length} videos.`);
      
      // Auto-fit timeline viewport to show all loaded videos
      if (loadedCount > 0) {
        const currentSegments = selectAllSegments(store.getState());
        if (currentSegments.length > 0) {
          const maxEnd = Math.max(...currentSegments.map(s => s.startTime + s.duration));
          const minStart = Math.min(...currentSegments.map(s => s.startTime));
          dispatch(fitTimelineToContent({
    ...
    const handleKeyDown = (e) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selection) {
        if (selection.type === 'segment') {
          ...
          const segmentToDelete = segments.find(seg => seg.id === selection.id);
          
          if (segmentToDelete) {
            // Clean up object URL if it's a blob URL (uploaded video)
            if (segmentToDelete.source && segmentToDelete.source.startsWith('blob:')) {
            ...
            const remainingSegments = segments.filter(seg => seg.id !== selection.id);
            if (remainingSegments.length > 0) {
              const maxEnd = Math.max(...remainingSegments.map(s => s.startTime + s.duration));
              const minStart = Math.min(...remainingSegments.map(s => s.startTime));
              dispatch(fitTimelineToContent({
                minStart: Math.max(0, minStart),
                maxEnd: maxEnd
              }));
        ...
        await new Promise((resolve) => {
          ...
          video.onloadedmetadata = () => {
            ...
            if (duration && duration > 0 && !isNaN(duration)) {
              ...
              const lastSegment = currentSegments.length > 0
                ? [...currentSegments]
              ...
              console.log(`Adding segment: ${file.name} at startTime: ${startTime}s`);
              ...
              if (updatedSegments.length > 0) {
                const maxEnd = Math.max(...updatedSegments.map(s => s.startTime + s.duration));
                const minStart = Math.min(...updatedSegments.map(s => s.startTime));
                dispatch(fitTimelineToContent({
                  minStart: Math.max(0, minStart),
                  maxEnd: maxEnd
                }));
                console.log(`Updated timeline viewport to show segments from ${minStart}s to ${maxEnd}s`);
              }
              
              console.log(`Dispatched addSegment for: ${file.name}`);
            } else {
              console.warn(`Invalid duration for video: ${file.name}`, duration);
              URL.revokeObjectURL(videoUrl); // Clean up
            }
            resolve();
          };
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            ...
            <Button
              ...
              ariaLabel={playhead.playing ? "Pause playback" : "Play all segments"}
              className="text-xs sm:text-sm px-2 sm:px-4 py-1.5 sm:py-2"
            >
